...
class Veekls_API_Client_Public
{
    ...
    {
        $types = Veekls_API_Client_Public::vehicle_types();
    ...
    {
        return Veekls_API_Client_Public::translate_vehicle_type($vehicle->type);
    }
    ...
    public function title($title, $vehicle)
    {
        ...
        if ($id) {
            $title['title'] = trim($vehicle->brand) . ' ' . trim($vehicle->model) . ' ' . trim($vehicle->version);
        }
